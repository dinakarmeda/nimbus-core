      ...
      model.params.forEach(element => {
        if (element.config && element.config.uiStyles) {
          if (
            ...
            element.config.uiStyles.attributes.alias === 'Accordion'
          ) {
            if (!recursion) {
            ...
            element.config.uiStyles != null &&
            element.config.uiStyles.attributes.alias === 'FormElementGroup'
          ) {
            if (!recursion) {
              let currElement = {} as FormModel;
  ...
  buildFormElements(model: Model) {
    ...
    this.subscribers.push(
      this.pageSvc.eventUpdate$.subscribe(event => {
        if (
          event.config &&
          event.config.uiStyles != null &&
          event.config.uiStyles.attributes.alias === 'Form' &&
          event.path === this.element.path
        ) {
