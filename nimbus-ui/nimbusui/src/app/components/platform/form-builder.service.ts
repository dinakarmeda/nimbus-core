...
export class FormElementsService {
  ...
  toFormGroup(elements: Param[], formValidations: ValidatorFn[]) {
    ...
    group.paramConfigs = elements.map(x => x.config);
    return group;
  }
  ...
  buildFormGroup(elements: Param[]): {} {
    ...
    elements.forEach(element => {
      if (
        element.config != null &&
        ((element.config.uiStyles != null &&
          element.config.uiStyles.attributes != null &&
          element.config.uiStyles.attributes.dataEntryField) ||
          element.config.uiStyles == null)
      ) {
        ...
        if (
          element.type &&
          element.config.type.nested &&
          element.type.model.params.length > 0 &&
          ...
          if (checks) {
            group[element.config.code] = [
              { value: leafState, disabled: !element.enabled },
              checks
            ];
          } else {
            group[element.config.code] = [
  ...
  private _getTypeSafeLeafState(param: Param): any {
    ...
    if (ParamUtils.isKnownDateType(param.config.type.name)) {
      ...
      param.alias === 'Grid' &&
      param.tableBasedData.values &&
      param.tableBasedData.values.length > 0
    ) {
